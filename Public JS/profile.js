 ...
 $(document).unbind().ready(function(){ user_pro=user.user_profile;
			    //get other info
				 get_info(user.id);
...
//friends details 
         $.get(url_vrt+'profile/friends_of_pro/'+user.user_profile+'',function(o){
...
function create_info(user_profile,id,profile_image,firstName,lastName,bio,country){
 ...
 if(img_wdth>100){ img__wdth=100;img_height=(img_height/img_wdth)*img__wdth;if(img_height<100){img_height=100;}
 if(img_height>100){radius_border=25;top_margin= -(img_height/2)+5;}else{radius_border=50;top_margin= 0;}}else{img__wdth=img_wdth;
 if(img_height>100){radius_border=25;
...
function get_info(id){
	 $.get(url_vrt+'profile/get_user_info/'+id+'',function(o){
		 	 if(user_logged==user_pro){
...
function update(hint){	update_val=$('#input_val_'+hint+'').val();$.ajax({type: "POST",url: "profile/update_data/"+hint+"",data:{data:update_val},
cache: false,success: function(html){$('#material_'+hint+'').html(update_val);}});
}	
function run(hint){$('#editbox_'+hint+'').show();$('#material_'+hint+'').hide();var data=$('#material_'+hint+'').html();$('#input_val_'+hint+'').html(data);$('#input_val_'+hint+'').focus();$('#input_val_'+hint+'').expandable();
$('#input_val_'+hint+'').keyup(function (e){ if(e.keyCode == 13){var curr = getCaret(this);var val = $(this).val();var end = val.length; $(this).val( val.substr(0, curr) + '<br>' + val.substr(curr, end));}
        });
